...
impl ServerKey {
    ...
    pub fn exec_gadget_with_extraction(
        ...
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.exec_gadget_with_extraction(enc_in, enc_inter, enc_out, input, &self)
        })
    }
    //////////////////////////////////////////////////
    ...
    pub fn apply_lut(
        ...
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.apply_lut(input, encoding_out, f, self)
        })
    }
    ///////////////////////////////////////////////////
        ...
        let origin_submodulis: Vec<u64> = inputs
            ...
            .map(|c| match c {
                ...
                Ciphertext::Trivial(_) => {
                    panic!("No tree bootstrapping with trivial ciphertexts (yet)")
                }
            })
            .collect();
    ...
    pub fn full_tree_bootstrapping_three_blocks(
        &self,
        inputs: &Vec<Ciphertext>,
        encodings_out: &Vec<Encoding>,
        t: u64,
        ...
        let common_factor = BooleanEngine::with_thread_local_mut(|engine| {
            engine.compute_common_factor(&inputs[2], &encodings_out[0], &self)
        });

        let r0 = BooleanEngine::with_thread_local_mut(|engine| {
            engine.simple_tree_bootstrapping_three_blocks(
                &common_factor.clone(),
                inputs,
                &encodings_out[0],
                t,
        ...
        let r2 = BooleanEngine::with_thread_local_mut(|engine| {
            engine.simple_tree_bootstrapping_three_blocks(
                &common_factor.clone(),
                inputs,
                &encodings_out[2],
    ...
    pub fn encoding_switching_lut(
        &self,
        input: &Ciphertext,
        encoding_out: &Encoding,
    ) -> Ciphertext {
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.apply_lut(input, encoding_out, &|x| x, self)
        })
    }
    ...
    pub fn encoding_switching_mul_constant(
        ...
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.encoding_switching_mul_constant(input, coefficient, &self)
        })
    }
    ...
    pub fn encoding_switching_sum_constant(
        ...
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.encoding_switching_sum_constant(input, constant, modulus, &self)
        })
    }
    ////////////////////////
    ...
    pub fn simple_plaintext_sum(
        ...
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.simple_plaintext_sum(input, constant, modulus, &self)
        })
    }

    pub fn simple_mul_constant(&self, input: &Ciphertext, coeff: u64, modulus: u64) -> Ciphertext {
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.simple_mul_constant(input, coeff, modulus, &self)
        })
    }
    ...
    pub fn linear_combination(
        ...
        let buffer: Vec<Ciphertext> = input
            .iter()
            .zip(coefficients)
            .map(|(ct, coeff)| self.simple_mul_constant(ct, *coeff, modulus))
            .collect();

        BooleanEngine::with_thread_local_mut(|engine| engine.simple_sum(&buffer, self))
    }
}
