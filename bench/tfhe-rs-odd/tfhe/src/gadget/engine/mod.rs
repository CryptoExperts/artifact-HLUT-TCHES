...
impl BooleanEngine {
    ...
    fn decryption_into_encoded_plaintext(
        &mut self,
        cks: &ClientKey,
        ciphertext: &Ciphertext,
    ) -> Plaintext<u64> {
        let (lwe_sk, _) = match cks.parameters.encryption_key_choice {
            EncryptionKeyChoice::Big => (
                cks.glwe_secret_key.as_lwe_secret_key(),
                cks.parameters.glwe_modular_std_dev,
            ),
        ...
        let ct = match ciphertext {
            Ciphertext::EncodingEncrypted(lwe_ciphertext, _) => lwe_ciphertext,
            Ciphertext::Trivial(_) => panic!("Should not have been called with a trivial one"),
        };

        decrypt_lwe_ciphertext(&lwe_sk, ct)
    }
    ...
    pub fn decrypt(&mut self, ct: &Ciphertext, cks: &ClientKey) -> u64 {
        match ct {
            ...
            Ciphertext::EncodingEncrypted(_, encoding) => {
                self.decrypt_with_encoding(&cks, ct, encoding)
            }
        }
    }
    ...
    pub fn decrypt_with_encoding(
        ...
        // decryption
        let decrypted = self.decryption_into_encoded_plaintext(&client_key, ciphertext);
        ...
        // println!("floating result:{}", floating_result);
        // println!("closest_integer:{}", closest_integer);
        ...
        // panic!(
        //     "No value in Zo has been found for : {} in encoding {:?}.",
        //     floating_result, encoding
        // );
        return 0;
    }
...
impl BooleanEngine {
    ...
    pub fn simple_tree_bootstrapping_three_blocks(
        &mut self,
        common_factor_1: &GlweCiphertextOwned<u64>,
        inputs: &Vec<Ciphertext>,
        encoding_out: &Encoding,
        t: u64,
        lut_fi: Vec<u64>,
        ...
        let c_0 = inputs[2].clone();
        match c_0 {
            Ciphertext::EncodingEncrypted(lwe_ciphertext_0, encoding_in_0) => {
                let bootstrapper = &mut self.bootstrapper;
                let o_0 = encoding_in_0.get_origin_modulus();
                let mut first_functions: Vec<Vec<Vec<u64>>> = vec![];
                for i in 0..o_0 {
                    first_functions.push(vec![]);
                    for j in 0..o_0 {
                        first_functions[i as usize].push(
                            (0..o_0)
                                .map(|x| lut_fi[(x + j * o_0 + i * o_0 * o_0) as usize])
                                .collect(),
                        );
                    }
                }
                ...
                match server_key.pbs_order {
                    ...
                    PBSOrder::KeyswitchBootstrap => {
                        let lwe_c_0_after_ks = server_key.keyswitch(&lwe_ciphertext_0);
                        let mut accumulators = vec![];
                        for i in 0..o_0 as usize {
                            let first_ciphertexts_i = bootstrapper
                                .mvb_bootstrap_with_common_factor_given(
                                    common_factor_1,
                                    lwe_c_0_after_ks.clone(),
                                    &encoding_in_0,
                                    &vec![encoding_out.clone(); o_0 as usize],
                                    &first_functions[i],
                                    &server_key,
                                    client_key_debug,
                                );
                            accumulators.push(bootstrapper.pack_into_new_accumulator(
                                first_ciphertexts_i,
                                server_key,
                                encoding_in_0.get_modulus(),
                            ));
                        ...
                        let c_1 = inputs[1].clone();
                        match c_1 {
                            Ciphertext::EncodingEncrypted(lwe_c_1, encoding_in_1) => {
                                match server_key.pbs_order {
                                    PBSOrder::BootstrapKeyswitch => {
                                        panic!()
                                    }
                                    PBSOrder::KeyswitchBootstrap => {
                                        let lwe_c_1_after_ks = server_key.keyswitch(&lwe_c_1);
                                        let second_ciphertexts = accumulators
                                            .iter()
                                            .map(|accumulator| {
                                            ...
                                            .collect();
                                        let next_accumulator = bootstrapper
                                            .pack_into_new_accumulator(
                                                second_ciphertexts,
                                                server_key,
                                                encoding_in_1.get_modulus(),
                                            );
                                        let c_2 = inputs[0].clone();
                                        match c_2 {
                                            Ciphertext::EncodingEncrypted(lwe_c_2, _) => {
                                                //we assume that they both hve the same input encoding
                                                let lwe_c_2_after_ks =
                                                    server_key.keyswitch(&lwe_c_2);
                                                let final_lwe = bootstrapper.bootstrap(
                                                    &lwe_c_2_after_ks,
                                                    &next_accumulator,
                                                    server_key,
                                                );
                                                Ciphertext::EncodingEncrypted(
                                                    final_lwe,
                                                    encoding_out.clone(),
                                                )
                                            }
