...
impl ClientKey {
    ...
    pub fn encrypt_arithmetic(&self, message: u64, encoding: &Encoding) -> Ciphertext {
        ...
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.encrypt_arithmetic(message, encoding, self)
        })
    }
    ...
    pub fn decrypt_arithmetic(&self, ct: &Ciphertext, encoding: &Encoding) -> u64 {
        BooleanEngine::with_thread_local_mut(|engine| {
            engine.decrypt_with_encoding(self, ct, encoding)
        })
    }
