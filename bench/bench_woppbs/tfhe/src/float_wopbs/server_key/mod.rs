...
impl ServerKey {
    ...
    pub fn new(cks: &ClientKey) -> ServerKey {
        ...
        let sks = shortint::server_key::ServerKey::new_with_max_degree(&cks.key, MaxDegree(max));
        // TODO DANGER
        let wopbs_key = shortint::wopbs::WopbsKey::new_wopbs_key_only_for_wopbs(&cks.key, &sks);
        ServerKey {
            key: sks,
            wopbs_key,
        }
    ...
    pub fn create_lut_for_benchmark(&self, ct: &mut Vec<ShortintCiphertext>) -> Vec<Vec<u64>> {
        let number_of_blocks = ct.len();
        let total_bits = f64::log2((self.key.message_modulus.0) as f64) as usize;
        let mut lut_len = 1 << total_bits;
        if 1 << total_bits < self.wopbs_key.param.polynomial_size.0 as u64 {
            lut_len = self.wopbs_key.param.polynomial_size.0;
        }


        let vec_lut = vec![vec![0; lut_len]; number_of_blocks];
        vec_lut
    }
    ...
    fn wop(
        ...
        for ct_in in vec_ct_in.iter_mut() {
            ...
            for block in ct_in.ct_vec_float.iter_mut() {
                ...
                // TODO DANGER
                let tmp = self.wopbs_key.extract_bits(
                    delta_log,
                    block,
                    parameters::ExtractedBitsCount(nb_bit_to_extract),
                );
    ...
    pub fn wop_pbs_to_benchmark(
        &self,
        _sks: &ServerKey,
        vec_ciphertexts: &mut Vec<ShortintCiphertext>,
        lut: &[Vec<u64>],
        total_bits_to_extract: usize,
        ...
        let mut vec_lwe = LweCiphertextList::new(
            0u64,
            extract_bits_output_lwe_size,
            LweCiphertextCount(total_bits_to_extract),
            self.wopbs_key.param.ciphertext_modulus,
        );
        ...
        // Extraction of each bit for each block
        let mut remain_bit_to_extract = total_bits_to_extract;
        let mut nb_bit_to_extract_per_block =
            f64::log2((self.key.message_modulus.0) as f64) as usize;
        for block in vec_ciphertexts.iter_mut() {
            let delta = (1_usize << 63) / (block.message_modulus.0 * block.carry_modulus.0);
            let delta_log = DeltaLog(f64::log2(delta as f64) as usize);
            if nb_bit_to_extract_per_block > remain_bit_to_extract {
                nb_bit_to_extract_per_block = remain_bit_to_extract;
                remain_bit_to_extract = 0;
            } else {
                remain_bit_to_extract -= nb_bit_to_extract_per_block
            }

            // Fill in reverse to have the proper order for float ops
            bits_extracted_so_far += nb_bit_to_extract_per_block;
            let extract_from_bit = total_bits_to_extract - bits_extracted_so_far;
            let extract_to_bit = extract_from_bit + nb_bit_to_extract_per_block;

            // TODO DANGER
            let tmp = self.wopbs_key.extract_bits(
                delta_log,
                block,
                parameters::ExtractedBitsCount(nb_bit_to_extract_per_block),
            );
        ...
        let mut output: Vec<crate::shortint::Ciphertext> = vec![];
        for (block_mantissa, result_ct) in
        vec_ciphertexts.iter().zip(vec_ct_out.drain(..))
        {
            output.push(crate::shortint::Ciphertext::new(
                result_ct,
                Degree(block_mantissa.message_modulus.0 - 1),
                NoiseLevel::NOMINAL,
                block_mantissa.message_modulus,
